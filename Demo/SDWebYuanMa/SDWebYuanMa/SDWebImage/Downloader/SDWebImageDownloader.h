...
//SDWebImageDownloaderExecutionOrder定义了数据被调用的顺序。按照一般的想法。下载应该按照数据放入队列的顺序依次进行，但也支持后进先出这种方式。
//
//一个下载管理器应该这样管理下载，肯定有一个下载列表，我们可以假定这个列表保存在一个数组中，正常情况下我们应该每次取出数组中第1个元素来下载，这就是FIFO(先进先出)。那么我们要改为LIFO(后进先出)，应该是针对某一个下载的，不应该是把取出数据的顺序改为从数组的最后一个元素取出。
